...
Prim算法是一种用于寻找加权无向图的最小生成树的贪心算法。它的基本思想是从一个起始顶点开始，不断选择与当前树连接的权重最小的边，逐步扩展形成最小生成树。

#### 算法步骤
1. 选择任意起始顶点作为树的根节点
2. 循环执行以下步骤，直到所有顶点都被访问：
   - 在与当前树相邻的所有边中选择权重最小的边
   - 将这条边和它连接的未访问顶点添加到树中
...
void primMST(int graph[V][V]) {
    int parent[V];     // 存储MST
    int key[V];        // 存储构造MST时使用的key值
    bool mstSet[V];    // 标记已包含在MST中的顶点
...
Kruskal算法是另一种计算最小生成树的方法。它的基本思想是按照边的权重从小到大的顺序，逐步选择边加入生成树中，同时避免形成环。

#### 算法步骤
1. 将图中所有边按权重排序
2. 从权重最小的边开始，依次执行：
   - 如果当前边的两个顶点不在同一个集合中，则将这条边加入生成树
   - 使用并查集来判断顶点是否在同一个集合中
...
拓扑排序用于有向无环图(DAG)，它是对图中顶点的一种排序，使得所有的有向边均从排在前面的元素指向排在后面的元素。

#### 算法步骤
1. 计算每个顶点的入度
2. 将所有入度为0的顶点加入队列
3. 依次从队列中取出顶点，将其所有后继顶点的入度减1
4. 如果某个顶点的入度变为0，则将其加入队列
5. 重复步骤3和4直到队列为空
...
Kosaraju算法用于查找有向图中的强连通分量（SCC）。强连通分量是图中的一个最大子图，其中任意两个顶点之间都存在相互可达的路径。

#### 算法步骤
1. 对原图进行深度优先搜索，记录顶点的完成时间
2. 将图中所有边反向，得到转置图
3. 按照完成时间从大到小的顺序，对转置图进行深度优先搜索
4. 每次深度优先搜索访问到的顶点集合就是一个强连通分量
...
// DFS函数用于填充顶点的完成时间栈
void fillOrder(int graph[MAX_VERTICES][MAX_VERTICES], int v, bool visited[], 
               int* stack, int* stackSize, int V) {
    visited[v] = true;

    for (int i = 0; i < V; i++) {
        if (graph[v][i] && !visited[i]) {
            fillOrder(graph, i, visited, stack, stackSize, V);
        }
    }
...
// DFS函数用于打印强连通分量
void DFS(int graph[MAX_VERTICES][MAX_VERTICES], int v, bool visited[], int V) {
    visited[v] = true;
    printf("%d ", v);
...
// 获取图的转置
void getTranspose(int graph[MAX_VERTICES][MAX_VERTICES], 
                 int transpose[MAX_VERTICES][MAX_VERTICES], int V) {
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) {
...
// 打印强连通分量
void printSCCs(int graph[MAX_VERTICES][MAX_VERTICES], int V) {
    ...
    getTranspose(graph, transpose, V);
...
### 12. Ford-Fulkerson算法（最大流）

Ford-Fulkerson算法用于解决网络流问题中的最大流问题。该算法通过不断寻找增广路径来增加网络的流量，直到无法找到更多的增广路径为止。

#### 算法步骤
1. 初始化残余网络，所有边的流量为0
2. 在残余网络中寻找从源点到汇点的增广路径
3. 计算增广路径上的可增加流量（即路径上的最小残余容量）
4. 更新残余网络中的流量
5. 重复步骤2-4直到找不到增广路径
...
int fordFulkerson(int graph[V][V], int s, int t) {
    ...
    int parent[V];  // 存储BFS构造的增广路径
    int max_flow = 0;  // 存储最大流
...
int main() {
    ...
    printf("The maximum possible flow is: %d\n", 
           fordFulkerson(graph, 0, 5));  // 源点为0，汇点为5
...
快速排序是一种高效的、基于分治策略的排序算法，平均时间复杂度为O(nlogn)。

#### 算法原理：
1. 选择一个基准元素（pivot）
2. 将数组分区，使得所有小于基准的元素在左边，大于基准的元素在右边
3. 递归地对左右两个子区间进行快速排序
...
归并排序是一种稳定的分治排序算法，它将数组分成两半，分别排序后再合并。
...
基数排序是一种非比较型整数排序算法，它根据数字的每个位上的值，将要排序的元素分配到桶中，再收集回来。
...
#### 复杂度分析：
- 时间复杂度：O(d*(n+k))，其中d是最大数的位数，k是每位数的范围
- 空间复杂度：O(n+k)
- 稳定性：稳定

### 5. 计数排序（Counting Sort）

计数排序是一种适用于一定范围内的整数排序算法，它的核心思想是统计每个数出现的次数。
...
二分查找是一种高效的查找算法，适用于已排序的数组。其基本思想是通过将数组分成两半来缩小搜索范围。

#### 算法步骤
1. 确定数组的中间位置
2. 将待查找值与中间元素比较
3. 如果相等，则找到目标值
4. 如果目标值较小，则在左半部分继续查找
5. 如果目标值较大，则在右半部分继续查找
...
KMP算法是一种高效的字符串匹配算法，能够在文本字符串中快速找到模式串的位置。

#### 算法原理
KMP算法的核心是利用已经部分匹配这个有效信息，通过一个next数组，保存模式串中前后最长公共元素的长度，每次匹配失败时，通过next数组快速找到下一个匹配点。

#### 代码实现
```c
void computeLPSArray(char* pat, int M, int* lps) {
    int len = 0;  // 上一个最长相同前后缀的长度
    int i = 1;
    lps[0] = 0;  // lps[0]必须为0
...
Boyer-Moore算法是一种高效的字符串搜索算法，它通过从右向左匹配，并根据匹配失败的字符跳过尽可能多的字符来提高效率。

#### 算法原理
Boyer-Moore算法使用两个启发式规则：
1. 坏字符规则：当文本中的字符与模式不匹配时，根据这个字符在模式中最右出现的位置来移动模式
2. 好后缀规则：当发生不匹配时，移动模式使其与已匹配的后缀对齐
...
Rabin-Karp算法是一种基于散列的字符串搜索算法，它通过计算hash值来进行字符串匹配。

#### 算法原理
1. 计算模式串的hash值
2. 计算文本中每个长度为m的子串的hash值
3. 如果hash值相等，则进行字符串比较确认
...
| KMP         | O(n + m)       | O(n + m)       | O(m)       | 字符串精确匹配             |
| Boyer-Moore | O(n/m)         | O(nm)          | O(1)       | 字符串搜索，特别是长模式串 |
| Rabin-Karp  | O(n + m)       | O(nm)          | O(1)       | 多模式串匹配，文本指纹     |

### 使用建议

1. **二分查找**：适用于已排序的数组，是最基本且最常用的搜索算法。

2. **KMP算法**：适用于需要在文本中查找单个模式串的情况，特别是当模式串较短时。

3. **Boyer-Moore算法**：适用于大文本和长模式串的搜索，在实际应用中通常比KMP算法更快。

4. **Rabin-Karp算法**：适用于多模式串匹配，例如在文档中搜索多个关键词。
